...
void fdevent_context_poll::Loop() {
    ...
    // Notify waiting threads that the loop has exited
    NotifyLoopExit();
}
...
void fdevent_context_poll::Interrupt() {
    ...
    if (rc == 0) {
        std::cout << "fdevent interrupt fd was closed?" << std::endl;
    } else if (rc == -1 && errno != EAGAIN) {
        std::cout << "failed to write to fdevent interrupt fd" << std::endl;
    }
}
