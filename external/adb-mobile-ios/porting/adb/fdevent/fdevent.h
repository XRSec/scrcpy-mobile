...
struct fdevent_context {
  ...
  public:
    ...
    // Wait for the loop to actually exit (after TerminateLoop is called)
    void WaitForLoopExit();

    // Notify waiting threads that the loop has exited (called from Loop implementations)
    void NotifyLoopExit();
  ...
  protected:
    ...
    // Synchronization for waiting on loop exit
    std::mutex loop_exit_mutex_;
    std::condition_variable loop_exit_cv_;
...
// Porting: Wait for the fdevent loop to exit after TerminateLoop is called
void fdevent_wait_for_loop_exit();

// Porting: Get the global fdevent context
