...
InstallStatus install_listener(const std::string& local_name, const char* connect_to,
    ...
    printf("[adb_porting] install_listener: %s, fd: %d\n", listener->local_name.c_str(), listener->fd);
    if (listener->fd < 0) {
...
void reinstall_all_listeners() EXCLUDES(listener_list_mutex) {
    std::lock_guard<std::mutex> lock(listener_list_mutex);
    printf("[adb_porting] reinstall_all_listeners\n");
    // Print listeners count
    printf("[adb_porting] listeners count: %zu\n", listener_list.size());
    for (auto& l : listener_list) {
        printf("[adb_porting] reinstall_all_listeners: %s\n", l->local_name.c_str());
        std::string* error = nullptr;
        int resolved = 0;
        l->fd = socket_spec_listen(l->local_name, error, &resolved);
        LOG(INFO) << "+ reinstalled listener: " << l->local_name << ", fd: " << l->fd;
        if (error) {
            LOG(ERROR) << "reinstall_all_listeners: " << *error;
            continue;
        }
    }
}